...
- 基于精准DOM层级结构，模糊匹配随机类名，稳定修改消息容器的内边距
- 将消息容器的左右内边距调整为10px，提升阅读体验
- 使用CSS注入和JavaScript兜底双重机制，确保在动态加载内容时仍能生效
- 仅在 https://www.doubao.com/* 页面生效
...
- 使用 `GM_addStyle` 注入自定义CSS，基于完整DOM层级精准定位目标容器
- 采用模糊匹配类名的方式（如 `[class*="inter-"]`），避免因类名随机变化导致失效
- 实现JavaScript兜底逻辑，通过 `MutationObserver` 监听DOM变化，确保动态加载的内容也能被正确处理
- 仅匹配消息列表内的目标容器，避免全局样式污染
